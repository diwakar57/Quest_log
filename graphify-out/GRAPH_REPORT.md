...
## Corpus Check
- 43 files · ~31,965 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 273 nodes · 482 edges · 24 communities (18 shown, 5 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `cc11ab59`
...
1. `playError()` - 22 edges
2. `Today()` - 19 edges
3. `playClick()` - 18 edges
4. `Execution Workflow — Task 1 to Task 15` - 18 edges
5. `Quest Log — Build Tasks for Claude Code` - 16 edges
6. `todayISODate()` - 14 edges
...
- `BottomNav()` --calls--> `playClick()`  [EXTRACTED]
  src/App.jsx → src/lib/sound.js
- `App()` --calls--> `setSoundEnabled()`  [EXTRACTED]
  src/App.jsx → src/lib/sound.js
...
### Community 6 - "playError"
Cohesion: 0.15
Nodes (32): BottomNav(), getAudioContext(), playClick(), playComplete(), playError(), playLevelUp(), playTone(), playTrialCleared() (+24 more)
...
Cohesion: 0.19
Nodes (16): react, App(), DebugDateBanner(), activeDebugDate(), DAYS, pickRunDay(), todayISODate(), supabase (+8 more)
...
Cohesion: 0.36
Nodes (9): checkAchievements(), checkCondition(), countRows(), countRunQuestsCompleted(), getBestStreak(), getCurrentTier(), hasPerfectWeek(), RUN_QUEST_NAMES (+1 more)
...
### Community 22 - "Character.jsx"
Cohesion: 0.35
Nodes (8): achievements, levelForXp(), totalXpFrom(), XP_FIELDS, xpForLevel(), Character(), rankForLevel(), STATS

### Community 23 - "streakLogic.js"
Cohesion: 0.39
Nodes (6): computeMultiplier(), computeStreak(), formatUTCDate(), isDayComplete(), parseUTCDate(), refreshStreak()

## Knowledge Gaps
- **107 isolated node(s):** `$schema`, `oxc`, `react/rules-of-hooks`, `warn`, `name` (+102 more)
...
- **Why does `react` connect `App.jsx` to `.oxlintrc.json`, `Today.jsx`, `Character.jsx`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `plugins` connect `.oxlintrc.json` to `App.jsx`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `Today()` connect `playError` to `App.jsx`, `Today.jsx`, `streakLogic.js`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
